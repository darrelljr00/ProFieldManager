...
- **OBD GPS Fuel Tracking Integration**: Extended fuel cost calculation system to capture total daily driven miles from OBD GPS trips database for comprehensive fleet fuel analysis. New endpoint `/api/dispatch/obd-fuel-usage` aggregates all vehicle miles from OBD trips (not just job-to-job segments) and applies fuel cost formula across daily, weekly, and monthly time periods. Gas & Maintenance report now includes data source toggle to switch between "Job Routes" (travel segments) and "OBD Trips" (total vehicle miles), with view selector for daily/weekly/monthly OBD aggregations. Both modes use same fuel price fallback logic and variance analysis, providing complete visibility into actual vs calculated fuel costs whether tracking job-specific routing or total fleet mileage.
- **Daily Fuel Usage Calculation System**: Implemented intelligent fuel cost calculation based on actual job routing and vehicle efficiency. System calculates fuel costs using formula: (total miles driven per day ÷ vehicle MPG) × fuel price per gallon, then compares with actual expense receipts to show variance analysis. Fuel pricing uses three-tier fallback: (1) same-day OCR'd expense receipts, (2) organization average from recent expenses, (3) default $3.50. New "Daily Fuel Usage Analysis" section in Gas & Maintenance report displays calculated vs actual costs with color-coded variance indicators (red = over budget, green = savings). Backend endpoint `/api/dispatch/daily-fuel-usage` aggregates travel segments by date/vehicle for accurate daily fuel consumption tracking.
- **GPS Arrival & Time Tracking Complete**: Fully operational GPS geofencing arrival detection (200m radius) and automated job time exceeded monitoring system with timestamp tracking. System stores arrivedAt and timeExceededAt on jobs, sends notifications to admins/managers with throttling (1-hour for arrivals, 2-hour for time exceeded), and displays GPS/time data in profit/loss reports. Uses orderBy(desc) and manual timestamp comparison for Neon PostgreSQL compatibility. Profit/loss per-job view now includes GPS Arrival and Time Status columns showing arrival timestamps and on-time/exceeded indicators.
- **Advanced Notification System with Visual Alerts**: Implemented notification system for admin/manager oversight with four active automated notification types: (1) Job Started, (2) Job Completed, (3) GPS Arrival Detection, (4) Job Time Exceeded. Features a flashing/pulsing CSS animation (notification-flash keyframe) on the notification badge in the sidebar to draw attention to unread notifications. All notifications target admin/manager roles exclusively and use 'team_based' category for proper organization.
