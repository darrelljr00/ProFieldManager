...
- June 29, 2025. Implemented universal image compression system using Sharp library that applies to ALL uploaded images across the entire platform - moved compression settings from 'inspection' to 'system' category for global application, updated all image upload endpoints (logo uploads, file uploads, image gallery uploads, and inspection image uploads) to automatically apply configurable compression with quality control (10-100%), dimension limits (100-4000px), and enable/disable toggle accessible through Admin Settings
