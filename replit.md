...
- **Deploy to CWP System**: Complete deployment infrastructure for deploying the application to Control Web Panel (CWP) servers. Features include: SSH/rsync-based file transfer, real-time log streaming via Server-Sent Events (SSE), deployment history tracking with PostgreSQL `deployments` table, terminal-style log viewer with live updates, server configuration management (host, user, port, remote path), DEPLOY_API_TOKEN authentication, setup guide with SSH key configuration instructions. Admin-only access via `/deploy-cwp` route. Environment variables: `DEPLOY_API_TOKEN` (required for authentication), `CWP_SSH_PRIVATE_KEY` (SSH private key content), `CWP_SSH_KEY_PATH` (alternative path to SSH key file).
- **Puzzle Picture Captcha**: Custom slider-puzzle captcha for login security. Features include: SVG-based puzzle generation with randomized patterns and colors, drag-and-drop puzzle piece interaction, mobile touch support, auto-generated challenges with server-side validation, token-based verification with 60-second expiration, 5-pixel tolerance for accessibility, automatic cleanup of expired challenges, and refresh capability. Located at `server/services/puzzleCaptcha.ts` (backend) and `client/src/components/PuzzleCaptcha.tsx` (frontend).
- **Comprehensive SEO Implementation**: Full search engine optimization for public-facing pages targeting field service management keywords. Features include: centralized SEO configuration (`client/src/lib/seo-config.ts`) with page-specific metadata, enhanced SEOHead component with canonical URLs, Twitter Cards, Open Graph tags, and JSON-LD structured data support, dynamic sitemap.xml and robots.txt generation via backend endpoints (`server/seo-routes.ts`), industry-specific landing pages with targeted keywords (HVAC, Plumbing, Electrical, Construction, General Contractors, Handyman, Pressure Washers, Window Washers, Service Technicians), Organization and SoftwareApplication schema.org structured data for rich search results.
