...
- **Advanced Notification System with Visual Alerts**: Implemented notification system for admin/manager oversight with two active automated notification types: (1) Job Started - alerts when technicians start jobs, (2) Job Completed - alerts when jobs finish. Features a flashing/pulsing CSS animation (notification-flash keyframe) on the notification badge in the sidebar to draw attention to unread notifications. All notifications target admin/manager roles exclusively and use 'team_based' category for proper organization. GPS geofencing for arrival notifications and automated job time exceeded checker are temporarily disabled due to SQL timestamp comparison compatibility issues and will be re-enabled in a future update.
