...
export function FilePermissionsDialog({ isOpen, onClose, file }: FilePermissionsDialogProps) {
  ...
  const { data: filePermissions = [], isLoading: permissionsLoading } = useQuery({
  ...
  const createPermissionMutation = useMutation({
    mutationFn: async (permissionData: any) => {
      return apiRequest('POST', `/api/files/${file?.id}/permissions`, permissionData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/files', file?.id, 'permissions'] });
      toast({
        title: "Permission Created",
        description: "File permission has been created successfully.",
      });
      resetForm();
    },
    onError: (error: any) => {
  ...
  const updatePermissionMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: number; updates: any }) => {
      return apiRequest('PUT', `/api/file-permissions/${id}`, updates);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/files', file?.id, 'permissions'] });
      toast({
        title: "Permission Updated",
        description: "File permission has been updated successfully.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const deletePermissionMutation = useMutation({
    mutationFn: async (permissionId: number) => {
      return apiRequest('DELETE', `/api/file-permissions/${permissionId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/files', file?.id, 'permissions'] });
      toast({
        title: "Permission Deleted",
        description: "File permission has been deleted successfully.",
      });
    },
    onError: (error: any) => {
      toast({
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            File Permissions - {file?.fileName}
          </DialogTitle>
        </DialogHeader>
          ...
          <div>
            ...
            {permissionsLoading ? (
              ...
              <div className="space-y-3">
                {filePermissions.map((permission: Permission) => (
