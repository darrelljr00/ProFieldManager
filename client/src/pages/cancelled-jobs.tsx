...
import { XCircle, ArrowLeft, User, Calendar, MapPin, DollarSign, Undo2 } from "lucide-react";
...
export function CancelledJobs() {
  ...
  const { data: cancelledJobs = [], isLoading } = useQuery<CancelledProject[]>({
    queryKey: ["/api/projects/cancelled"],
  });
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Invalidate and refetch both cancelled jobs and regular projects
  ...
  const formatAddress = (project: CancelledProject) => {
    ...
    return (
      <div className="p-6">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Cancelled Jobs</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-2">Jobs that have been cancelled</p>
        </div>
        <div className="text-center py-8">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div>
          <p className="mt-2 text-gray-600 dark:text-gray-300">Loading cancelled jobs...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-300">
          Jobs that have been cancelled ({cancelledJobs.length} jobs)
        </p>
      </div>
      ...
      {cancelledJobs.length === 0 ? (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Jobs that are cancelled will appear here. You can manage them separately from active jobs.
            </p>
            <Link href="/jobs">
              <Button>
                <ArrowLeft className="w-4 h-4 mr-2" />
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {cancelledJobs.map((job) => (
            <Card key={job.id} className="hover:shadow-lg transition-shadow border-orange-200 dark:border-orange-800">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  <div className="pt-2 border-t border-gray-200 dark:border-gray-700 space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-600 dark:text-gray-300">
                        Cancelled {format(new Date(job.updatedAt), "MMM d, yyyy")}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  >
                    ...
                    {restoreMutation.isPending ? "Restoring..." : "Undo Cancel"}
                  </Button>
                </div>
              </CardContent>
            </Card>
