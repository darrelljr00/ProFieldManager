...
export default function InternalMessagesPage() {
  ...
  const chatRooms: ChatRoom[] = users.filter(u => u.id !== user?.id).map(u => ({
    ...
    lastMessage: messages.filter(m => 
      m.senderId === u.id || (m.recipients && Array.isArray(m.recipients) && m.recipients.some(r => r && r.recipientId === u.id))
    ).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0],
    unreadCount: messages.filter(m => 
      m.senderId === u.id && 
      m.recipients && Array.isArray(m.recipients) && m.recipients.some(r => r && r.recipientId === user?.id && !r.isRead)
    ).length
  }));
  ...
  const roomMessages = selectedRoom && selectedRoom.participants[0] ? messages.filter(m => 
    ...
    (m.recipients && Array.isArray(m.recipients) && m.recipients.some(r => r && r.recipientId === selectedRoom.participants[0].id))
